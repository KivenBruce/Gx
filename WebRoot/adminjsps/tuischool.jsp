				...
				<div class="span9">
					<div class="row-fluid">
						<div class="page-header">
							...
							<c:if test="${msg=='校内推荐活动'}">
								<h1>
									${msg}<small><a href="tuiaddschool.jsp?type=In"
										style="margin-left: 10px">添加</a></small>
								</h1>
							</c:if>
							<c:if test="${msg=='校外推荐活动'}">
								<h1>
									${msg}<small><a href="tuiaddschool.jsp?type=Out"
										style="margin-left: 10px">添加</a></small>
								</h1>
							</c:if>
						</div>
						...
						<c:if test="${fn:length(type)>8 }">
							<c:set var="gotos" value='/OutServlet?' />
						</c:if>
						<c:if test="${fn:length(type)<9 }">
							<c:set var="gotos" value='/InServlet?' />
						</c:if>
										...
										<td>
											<div class="btn-group">
												<a class="btn btn-mini dropdown-toggle"
													onmouseover="showul()">管理活动 <span class="caret"></span>
												</a>
