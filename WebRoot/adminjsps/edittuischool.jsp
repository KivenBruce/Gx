			...
			<div class="span9">
				<div class="row-fluid">
					...
					<c:if test="${fn:length(type)<9 }">
						<c:set var="gotos" value='/InServlet?method=editSchool&gid=${gid}' />
					</c:if>

					<form onsubmit="return judge()" action="<c:url value='${ gotos}'/>" method="post">
						...
						<fieldset>
							...
							<c:if test="${fn:length(type)>8 }">
								<div style="float: right; margin-right: 190px; margin-top: -80px;">
									<input type="submit" class="btn btn-success btn-large"
										value="Save Changes" /> <a class="btn"
