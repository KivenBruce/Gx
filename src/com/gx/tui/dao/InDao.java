...
public class InDao {
	...
	public PageBean<School> findAll(int pc, String datefrom, String dateto, String place) {
		String sql = "select * from intuijian where Gtime>?  and Gtime<? and Gplace like ?  ORDER BY Gtime desc limit ?,?";
		String sql1 = "select count(*) from intuijian where Gtime>? and Gtime<? and Gplace like ?";
	...
	/*
	 * public List<accountID> queryID() throws SQLException { String sql =
	 * "select ID from  intuijian"; List<accountID> id =
	 * (List)this.qr.query(sql, new BeanListHandler(accountID.class)); return
	 * id; }
	 */

	public School findById(String gid) {
		String sql = "select * from intuijian where id=?";
		try {
	...
	public void addTuiSchool(School school) {
		String sql = "insert into intuijian(Gtheme,Gpart,Gtime,Gcontent,Gusername,Gperson,Gremain,Gtupian,Gprice,Gplace)values(?,?,?,?,?,?,?,?,?,?)";
	...
	public void delete(String gid) {
		String sql = "delete from intuijian where id=?";
		try {
	...
	/*public void editInDomain(School school, String gid) {
		String sql = "update intuijian set Gtheme=?,Gcontex=?,Glink=?,Gnumber=?,Gplace=?,Gtime=?,Gpicture=?  where id="
				+ gid;
		Object[] params = { school.getGtheme(), school.getGcontex(), InDomain.getGlink(), InDomain.getGnumber(),
				school.getGplace(), school.getGtime(), InDomain.getGpicture() };
		try {
			this.qr.update(sql, params);
		} catch (SQLException e) {
			throw new RuntimeException(e);
	...
	public void editSchool(School school, String gid) {
		String sql = "update intuijian set Gtheme=?,Gpart=? ,Gtime=? ,Gcontent=? ,Gusername=?,Gperson=?,Gplace=? ,Gtupian=? where id=?";
		Object[] params = { school.getGtheme(), school.getGpart(), school.getGtime(), school.getGcontent(),
